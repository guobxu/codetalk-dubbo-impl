...
public class PostServiceImpl extends AbstractAppService implements PostService {
	...
	public Long createPost(Post post) {
		...
		if(AppConstants.POST_TYPE_POLL == post.getType()) { // poll
	...
	public List<PostVO> searchByUserId(Long userId, Long beginDate, Long endDate, Integer page, Integer size) {
		List<String> followUsers = new ArrayList<String>(); // TODO
		List<String> tags = userTagService.findUserTagForSearch(userId);
		
		List<Long> postIdList = postSearchService.searchByUserAndTag(followUsers, tags, beginDate, endDate, page, size);
	...
	/*********************************** Service ***********************************/
	static final String URI_POST_DTL = "/flow/post";
	static final String URI_POST_CREATE = "/flow/post/create";
	static final String URI_POST_SEARCH = "/flow/post/search";
	
	static final String URI_POST_LISTBYUSER = "/flow/post/listbyuser";
	static final String URI_POST_LIKEBYUSER = "/flow/post/likebyuser";
	
	static final String URI_POST_LIKE = "/flow/post/like";
	...
	private String postDetail(Map<String, Object> params) {
		...
		Integer postType = Integer.parseInt((String)params.get("post_type"));
		
		if(postType != AppConstants.POST_TYPE_BASIC && postType != AppConstants.POST_TYPE_POLL) {
			return errParamMsg();
		}
		PostVO post = null;
...
//		enrichPosts(post);
	...
	private String postCreate(Map<String, Object> params) {
		...
		// poll 
		Map<String, Object> pollData = (Map<String, Object>)params.get("poll_data");
		if(pollData != null) {
			Poll poll = new Poll();
			long start = createDate, duration = Long.parseLong(pollData.get("poll_duration").toString()), end = start + duration;
			poll.setDuration(duration);
			poll.setStart(start);
			poll.setEnd(end);
			poll.setCreateDate(createDate);
			List<String> optionList = (List<String>)pollData.get("poll_options");
			List<PollOption> pollOptions = new ArrayList<>();
			for(String optionText : optionList) {
				PollOption option = new PollOption();
				option.setText(optionText);
				option.setCreateDate(createDate);
